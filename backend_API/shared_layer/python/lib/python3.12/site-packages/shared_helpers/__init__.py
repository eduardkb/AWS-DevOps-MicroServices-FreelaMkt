...
def get_cognito_sub(app):
    """Return the Cognito sub from the authenticated user."""

    # Production: API Gateway JWT authorizer
    if not IS_DEV:
        try:
            return (
    ...
    # Local SAM: decode the Authorization header
    try:
        auth = app.current_event.headers.get("authorization", "")
